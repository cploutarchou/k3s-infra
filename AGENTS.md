...
skills/           # portable skills + agent (canonical; sync.sh mirrors)
commands/         # reusable slash commands: health, triage, validate, sync-skills
scripts/          # validate.sh, kubeconfig minting, health report
docs/             # architecture, decisions, runbooks/
.claude/ .codex/  # per-tool settings, hooks, subagents (Claude Code, Codex)
.agents/skills/   # shared skills (Codex + ZCode plugin)
.agents/commands/ # shared slash commands (ZCode workspace + plugin)
.zcode/ .zcode-plugin/  # ZCode plugin: agents, hooks, MCP, manifest
```
