...
Prefer the `k3s-infra` MCP tools (`nodes`, `pods`, `events`, `logs`,
`flux_status`, `cnpg_status`, `ha_report`) for cluster state; they are
read-only by RBAC. Never call `flux_reconcile` or `propose_change` during
triage.
